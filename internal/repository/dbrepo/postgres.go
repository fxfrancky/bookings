...
// SearchAvailabilitiesByDatesByRoomID returns true if availability exist and false if no availability exist
func (m *postGresDBRepo) SearchAvailabilitiesByDatesByRoomID(start, end time.Time, roomID int) (bool, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	var numRows int
...
// SearchAvailabilitiesForAllRooms returns a slice of available rooms if any, for given date range
func (m *postGresDBRepo) SearchAvailabilitiesForAllRooms(start, end time.Time) ([]models.Room, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
// AllReservations returns a clice of all reservations
...
// AllNewReservations returns a clice of all reservations
