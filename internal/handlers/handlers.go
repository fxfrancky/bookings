...
import (
	...
	"github.com/fxfrancky/bookings/internal/forms"
	"github.com/fxfrancky/bookings/internal/helpers"
	...
	"github.com/fxfrancky/bookings/internal/repository/dbrepo"
...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	// send notifications email  to owner contact@owonafx.com
	htmlMessage = fmt.Sprintf(`
...
func (m *Repository) PostAvailability(w http.ResponseWriter, r *http.Request) {
	...
	rooms, err := m.DB.SearchAvailabilitiesForAllRooms(startDate, endDate)
	if err != nil {
...
func (m *Repository) AvailabilityJSON(w http.ResponseWriter, r *http.Request) {
	...
	availaible, err := m.DB.SearchAvailabilitiesByDatesByRoomID(startDate, endDate, roomID)
	if err != nil {
		// can't parse form, so return appropriate json
		resp := jsonResponse{
			OK:      false,
...
// ReservationSummary displays the reservation summarys page
...
// AdminAllNewReservations shows all new reservations in admin tool
...
// AdminShowReservation shows the reservation in the admin page
func (m *Repository) AdminShowReservation(w http.ResponseWriter, r *http.Request) {
	// check if we come from/new/if or /all/id
	exploded := strings.Split(r.RequestURI, "/")
	// http://localhost:8080/admin/reservations/new/1 id is the index 4 of the array split by /
	...
	// Show reservation from the database
...
// AdminPostShowReservation shows the reservation in the admin page
...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	// Gets first and last days of the month
...
// AdminPostReservationsCalendar handles post reservastion handler
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		...
		for name, value := range curMap {
			...
			if val, ok := curMap[name]; ok {
				// only pay attention to value >0, and that ar not in the form post
	...
	for name, _ := range r.PostForm {
		if strings.HasPrefix(name, "add_block") {
			...
			t, _ := time.Parse("2006-06-2", exploded[3])
			// insert a new block
